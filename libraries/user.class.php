...
class User{
	...
	function user(){
		if(!empty($_SESSION['user'])){
			if((time()-$_SESSION['start']) <  900 ){ //Auto Session Timeout at 15 Minutes (900 Sec)
				session_name("BUP_Hall");
	...
	function setUser($user = ''){
		...
		session_name("BUP_Hall");	//set session name
		$_SESSION['start'] = time(); //set session timeout
	}
	
	function getUser(){
		global $dbObj;
		if(session_name() != "BUP_Hall"){
			$this->userinfo = '';
		}
