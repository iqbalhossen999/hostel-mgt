...
switch($action){
	...
	case 'save':
		...
		$uploaded_s_photo = upload_file($_FILES['s_photo'], $path, $cur_user_id, 'jpg,jpeg,png,gif');
		$uploaded_student_signature = upload_file($_FILES['student_signature'], $path, $cur_user_id, 'jpg,jpeg,png,gif');
		$uploaded_g_photo = upload_file($_FILES['g_photo'], $path, $cur_user_id, 'jpg,jpeg,png,gif');
		$uploaded_g_signature = upload_file($_FILES['g_signature'], $path, $cur_user_id, 'jpg,jpeg,png,gif');
		
		if($cur_user_group_id == '3'){
			//Check for not inserting any unsupported format files
			if($uploaded_s_photo['error_counter'] != '0'){
				$msg = implode("<br />",$uploaded_s_photo['error']);
				$url = 'form.php?msg='.$msg.' for Student Photo';
				redirect($url);
			}
			
			if($uploaded_student_signature['error_counter'] != '0'){
				$msg = implode("<br />",$uploaded_student_signature['error']);
				$url = 'form.php?msg='.$msg.' for Student Signature';
				redirect($url);
			}
			
			if($uploaded_g_photo['error_counter'] != '0'){
				$msg = implode("<br />",$uploaded_g_photo['error']);
				$url = 'form.php?msg='.$msg.' for Guardian Photo';
				redirect($url);
			}
			
			if($uploaded_g_signature['error_counter'] != '0'){
				$msg = implode("<br />",$uploaded_g_signature['error']);
				$url = 'form.php?msg='.$msg.' for Guardian Signature';
				redirect($url);
			}
		}//if
		...
		if($cur_user_group_id == '3'){
			$fields = array('name' => $name,
						...
						//upload file
						's_photo' => $uploaded_s_photo['file'][0]['file_name'],
						'student_signature' => $uploaded_student_signature['file'][0]['file_name'],
						'g_photo' => $uploaded_g_photo['file'][0]['file_name'],
						'g_signature' => $uploaded_g_signature['file'][0]['file_name'],
			...
			if(!empty($id)){
				...
				if($cur_user_group_id == '3'){
					$fields1 = array(
							'full_name' => $name,
							'official_name' => $name,
							'photo' => $uploaded_s_photo['file'][0]['file_name'],
							'email' => $email
						);
	...
	<?php if($action=="insert"){ ?>
		...
		<form action="form.php" method="post" name="form" id="form" onsubmit="return validateForm(<?php echo $password_cond; ?>);" enctype="multipart/form-data">
			...
			<?php if($usr[0]->group_id != '1'){ ?>
				...
				<tr>
					<td height="30" colspan="2">
						<?php echo STUDENT_PHOTO; ?>:
						<input name="s_photo" id="s_photo" type="file" class="inputbox" alt="Photo" size="23" value="<?php echo $s_photo; ?>" />
					</td>
				</tr>
				<tr>
					<td height="30" colspan="2">
						<?php echo STUDENT_SIGNATURE; ?>:
						<input name="student_signature" id="student_signature" type="file" class="inputbox" alt="Student Signature" size="34" value="<?php echo $student_signature; ?>" />
					</td>
				</tr>
			<?php if(!empty($id)){ ?>
				<tr>
					<td align="center" height="200">
						<?php if(!empty($s_photo)){ ?>
							<img src="<?php echo $path.$s_photo; ?>" height="150" width="130" title="<?php echo $name; ?>" border="0" /><br />
						<?php }else{ ?>
							<img src="images/unknown.png" height="150" width="130" title="<?php echo $name; ?>" border="0" /><br />
						<?php }//if 
							echo $name;
						?>
					</td>
					<td align="center">
						<?php if(!empty($student_signature)){ ?>
							<img src="<?php echo $path.$student_signature; ?>" height="40" width="150" title="<?php echo 'Signature of '.$name; ?>" border="0" /><br />
						<?php }else{ 
							echo NO_SIGNATURE_ATTACHED.'<br /><br /><br />';
							}//if 
							echo SIGNATURE_OF.' '.$name; 
						?>
					</td>
				</tr>
			<?php }//if ?>
				...
				<tr>
					<td height="30" colspan="2">
						<?php echo GUARDIAN_PHOTO; ?>:
						<input name="g_photo" id="g_photo" type="file" class="inputbox" alt="Photo" size="23" value="<?php echo $g_photo; ?>" />
					</td>
				</tr>
				<tr>
					<td height="30" colspan="2">
						<?php echo GUARDIAN_SIGNATURE; ?>:
						<input name="g_signature" id="g_signature" type="file" class="inputbox" alt="Signature" size="36" value="<?php echo $g_signature; ?>" />
					</td>
				</tr>
			<?php if(!empty($id)){ ?>
				<tr>
					<td align="center" height="200">
						<?php if(!empty($g_photo)){ ?>
							<img src="<?php echo $path.$g_photo; ?>" height="150" width="130" title="<?php echo $g_name; ?>" border="0" /><br />
						<?php }else{ ?>
							<img src="images/unknown.png" height="150" width="130" title="<?php echo $g_name; ?>" border="0" /><br />
						<?php }//else 
							echo $g_name;
						?>
					</td>
					<td align="center">
						<?php if(!empty($g_signature)){ ?>
							<img src="<?php echo $path.$g_signature; ?>" height="40" width="150" title="<?php echo 'Signature of '.$g_name; ?>" border="0" /><br />
						<?php }else{ 
							echo NO_SIGNATURE_ATTACHED.'<br /><br /><br />';
							}//if 
							echo SIGNATURE_OF.' '.$name; 
